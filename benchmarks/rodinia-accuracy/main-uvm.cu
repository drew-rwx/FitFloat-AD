...
int main(int argc, char* argv[])
{
  ...
  int *d_label;
  cudaMallocManaged((void**)&d_label, label_size_bytes);
  
  float *d_data;
  cudaMallocManaged((void**)&d_data, data_size_bytes);
  
  int *d_count;
  cudaMallocManaged((void**)&d_count, sizeof(int));

  cudaMemcpy(d_label, label, label_size_bytes, cudaMemcpyHostToHost);
  cudaMemcpy(d_data, data, data_size_bytes, cudaMemcpyHostToHost);
  cudaDeviceSynchronize();
