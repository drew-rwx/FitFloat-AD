...
int main(int argc, char* argv[])
{
  ...
  double *d_u_new;
  cudaMallocManaged((void**)&d_u_new, grid_size);

  double *d_v_new;
  cudaMallocManaged((void**)&d_v_new, grid_size);

  double *d_u;
  cudaMallocManaged((void**)&d_u, grid_size);

  double *d_v;
  cudaMallocManaged((void**)&d_v, grid_size);

  cudaMemcpy(d_u_new, u_new, grid_size, cudaMemcpyHostToHost);
  cudaMemcpy(d_v_new, v_new, grid_size, cudaMemcpyHostToHost);
  cudaMemcpy(d_u, u, grid_size, cudaMemcpyHostToHost);
  cudaMemcpy(d_v, v, grid_size, cudaMemcpyHostToHost);
