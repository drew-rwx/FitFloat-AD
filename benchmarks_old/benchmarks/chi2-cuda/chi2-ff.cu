...
__global__ void chi_kernel(
  ...
  const long long cRows,
  const long long contRows,
...
int main(int argc, char* argv[]) {
  ...
  unsigned long long rows = 10000;
  unsigned long long cols = 200000;
  long long ncases = 100;
  long long ncontrols = 100;
  int nthreads = 512;
  int repeat = 1;
  ...
  FFArr32 d_results(result_size / 4);
