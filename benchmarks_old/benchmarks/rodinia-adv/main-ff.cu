...
int main(int argc, char **argv) {
  ...
  FFArr64 d_vgeo(Np*Nelements*p_Nvgeo);
  FFArr64 d_cubvgeo(cubNp*Nelements*p_Nvgeo);
  FFArr64 d_cubDiffInterpT(3*cubNp*Nelements);
  FFArr64 d_cubInterpT(Np*cubNp);
  FFArr64 d_u(3*Np*Nelements);
  FFArr64 d_adv(3*Np*Nelements);  

  DoubleH2FFDmemcpy(d_vgeo, vgeo, Np*Nelements*p_Nvgeo);
  DoubleH2FFDmemcpy(d_cubvgeo, cubvgeo, cubNp*Nelements*p_Nvgeo);
  DoubleH2FFDmemcpy(d_cubDiffInterpT, cubDiffInterpT, 3*cubNp*Nelements);
  DoubleH2FFDmemcpy(d_cubInterpT, cubInterpT, Np*cubNp);
  DoubleH2FFDmemcpy(d_u, u, 3*Np*Nelements);
  DoubleH2FFDmemcpy(d_adv, adv, 3*Np*Nelements);
