...
int main(int argc, char* argv[]) {
  ...
  const size_t num_elements = total_bytes / 4 * 7 / 10000 / 32 * 32;

  // image dimensions
  size_t w = sqrt(num_elements);
  size_t h = w;

  params p = {128, 3, 480, 640, 9, w, h};
  const int dim_b = p.output_dim_b;
  const int dim_c = p.output_dim_c;
  const int dim_h = p.output_dim_h;
  const int dim_w = p.output_dim_w;
  ...
  FFArr32 d_img(image_size);
  FloatH2FFDmemcpy(d_img, img, image_size);

  FFArr32 d_offsets_h(offset_size);
  FloatH2FFDmemcpy(d_offsets_h, offsets_h, offset_size);

  FFArr32 d_offsets_v(offset_size);
  FloatH2FFDmemcpy(d_offsets_v, offsets_v, offset_size);

  FFArr32 d_kernel(kernel_size);
  FloatH2FFDmemcpy(d_kernel, kernel, kernel_size);

  FFArr32 d_output(output_size);
