...
int main(int argc, char *argv[]) {
  ...
  const size_t num_elements = total_bytes / 4 * 5 / 10 / 2 / 32 * 32;
  ...
  FFArr32 d_src(img_size);
  FFArr32 d_dst(img_size);
  ...
  FloatH2FFDmemcpy(d_src, h_src, img_size);
