...
#include "blackScholesAnalyticEngineKernels-ff.cu"
...
void runBlackScholesAnalyticEngine(const int repeat)
{
  ...
  const size_t num_elements = total_bytes / 40 * 9 / 10 / 32 * 32;
  size_t numberOfSamples = num_elements;
  printf("Using %f GB\n", ((1.0 * numberOfSamples * sizeof(optionInputStruct)) + (1.0 * numberOfSamples * sizeof(float))) / 1000000000);

  {
    ...
    FFD2FloatHmemcpy(outputVals, outputValsGpu, numVals);
...
{
  runBlackScholesAnalyticEngine(1);
  return 0;
}
